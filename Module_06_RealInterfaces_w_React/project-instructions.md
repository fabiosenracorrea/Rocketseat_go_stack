...
Essa será uma aplicação que irá se conectar ao seu backend do [Desafio 05](../Module_05_DataBase_NodeJS), e exibir as transações criadas e permitir a importação de um arquivo CSV para gerar novos registros no banco de dados.
...
O layout pode ser acessado através da página do Figma (link protegido - sem disponibilidade pública para que eu poste aqui)
...
- **`Importar arquivos CSV`**: Na sua página `Import`, você deve permitir o envio de um arquivo no formato `csv` para o seu backend, que irá fazer a importação das transações para o seu banco de dados. O arquivo csv deve seguir um modelo específico.
